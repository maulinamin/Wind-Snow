...
%PLOT THE NORMAL PPP
p = polyfit(PPTH60.InvPi,PPTH60.TI,1); 
f = polyval(p,PPTH60.InvPi); 
figure;
subplot(2,2,1)
plot(PPTH60.InvPi,PPTH60.TI,'.',PPTH60.InvPi,f,'-')
grid on;
legend('data','linear fit')
dim = [0.2 0.5 0.3 0.3];
mdl = fitlm(PPTH60.InvPi,PPTH60.TI);
ylabel('Data(Xi)'); xlabel('Normal Inverse/Standard Normal Percentile');
X = sprintf('Normal PPP: %f X + %f and R-squared = %f',p(1),p(2),mdl.Rsquared.Ordinary);
title(X);
